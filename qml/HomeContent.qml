...
Rectangle {
    ...
    function mqttConnet(ip,port){

        console.debug("mqttConnet   "+ip + "    "+port)
        mqttwork.funMqttConnectServer(ip,port)
    }

    function mqttLogin(acc,pwd){

        var data={ "username":acc,"password":pwd};

        mqttwork.funMqttPublishMsg("loginmainserver",data);
